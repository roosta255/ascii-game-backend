...
int4 build_room_map(Array<Room, DUNGEON_ROOM_COUNT>& rooms, const iLayout& layout, std::unordered_map<int4, Pointer<Room>>& shifted) {
    ...
    while (!q.empty()) {
        if (i++ >= MAX_ITERATIONS) {
            LOG_ERROR << "build_room_map hit MAX_ITERATIONS";
            return int4{1,1,1,1};
        }

        auto [pos, ptr] = q.front(); q.pop();
        LOG_DEBUG << "build_room_map popped <" << pos[0] << ", " << pos[1] << ", " << pos[2] << ", " << pos[3] << ">";
            ...
            for (const auto dir: Cardinal::getAllCardinals()) {
                layout.getWallNeighbor(rooms, room, dir).accessConst([&](Room& neighbor){
                    ...
                    if (discovered.find(newPos) == discovered.end()) {
                        ...
                        LOG_DEBUG << "build_room_map pushed <" << newPos[0] << ", " << newPos[1] << ", " << newPos[2] << ", " << newPos[3] << ">, diff: " << (&neighbor - &rooms.head());
    ...
    for (auto& [pos, ptr] : discovered) {
        ...
        // LOG_DEBUG << "build_room_map shiftedPos<" << shiftedPos[0] << ", " << shiftedPos[1] << ", " << shiftedPos[2] << ", " << shiftedPos[3] << ">, offset: " << (ptr.getAddress() - &rooms.head());
    }
