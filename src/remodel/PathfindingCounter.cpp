...
bool PathfindingCounter::loopDoorways(const Array<Bitstick<DUNGEON_ROOM_COUNT>, 4>& excludedDoors, std::function<bool(const int&, const Cardinal&)> consumer, const int seed) const {
    static const auto DOOR_IDS = generateRandomArray<4>(356376);
    static const auto ROOM_IDS = generateRandomArray<DUNGEON_ROOM_COUNT>(2364332);
    bool isDone = false;
    for (int d = 0; d < 4 && !isDone; d++) {
        const auto dir = Cardinal(DOOR_IDS.getWindow(d + seed));
        doorwayMoveCounter.doors.accessConst(dir.getIndex(), [&](const Bitstick<DUNGEON_ROOM_COUNT>& allDoors){
            excludedDoors.accessConst(dir.getIndex(), [&](const Bitstick<DUNGEON_ROOM_COUNT>& excluded){
                const auto eligibleDoors = allDoors - excluded;
                for (int i = 0; i < DUNGEON_ROOM_COUNT && !isDone; i++) {
                    const auto index = ROOM_IDS.getWindow(i + seed);
                    if (eligibleDoors[index].orElse(false)) {
                        isDone = consumer(index, dir);
                    }
                }
            });
